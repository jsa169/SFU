...
def main():
    ...
    X_train, X_valid, y_train, y_valid = train_test_split(X, y, test_size=0.330)
    
    bayes_rgb_model= GaussianNB()

    bayes_convert_model =  make_pipeline(FunctionTransformer(convert),GaussianNB())

    knn_rgb_model = KNeighborsClassifier(n_neighbors=500)
    
    knn_convert_model = make_pipeline(FunctionTransformer(convert),KNeighborsClassifier(n_neighbors=200))
    
    rf_rgb_model = RandomForestClassifier(n_estimators=300,
                                                  max_depth=6,min_samples_leaf=10)
    
    rf_convert_model = make_pipeline(FunctionTransformer(convert),RandomForestClassifier(n_estimators=500,
                                                  max_depth=6,min_samples_leaf=10))
