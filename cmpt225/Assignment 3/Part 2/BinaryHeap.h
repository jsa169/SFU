...
void BinaryHeap<ElementType>::remove() throw(EmptyDataCollectionException)
{
    ...
    //If there is only one element, remove everything
    if(elementCount == 1){
...
//----------------------------------------------------------------------------------------------
   // Description: Prints out the binary heap. And the current element count  
   // Time Efficiency: O(n)
...
void BinaryHeap<ElementType>::reHeapDown(unsigned toBeReheaped) {
    ...
    if(left < elementCount){
        ...
        if((elements[minChild]) <= elements[toBeReheaped]){
